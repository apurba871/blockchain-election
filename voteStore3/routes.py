from voteStore3 import app, db
from flask import request
from voteStore3.models import Access, Share
import voteStore3.util as util
...
def store_vote():
  ...
  if election_id is None or part_no is None or share is None:
    ...
    util.store_vote_helper(election_id, part_no, share)
    return {"success":True}
  ...
  if "election_id" in request.args:
    ...
    if Access.is_present(election_id):
      if Access.get_has_access(election_id):
        ...
        return {"success":True, "election_id": election_id, "part_no":share.part_no, "share":share.share}
      else:
